...
1. Установите необходимые пакеты R (см. вначало скрипта)
2. Загрузите файл данных `wage_gap.xlsx`
3. Обновите путь к файлу в скрипте на вашу директорию
4. Запустите скрипт по секциям (обозначены `{#номер_пункта}`)

## Потенциальные улучшения

- Применение более продвинутых методов машинного обучения (случайные леса, нейронные сети)
- Анализ гетерогенных эффектов (различные эффекты для разных групп населения)
- Использование методов для обработки отказов от участия (attrition)
- Временной анализ долгосрочных эффектов переезда
